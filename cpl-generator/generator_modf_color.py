...
def write_colored_code(pdf, lexer, content):
    ...
    for token_type, token_string in tokens:
        ...
        pdf.set_font(pdf.code_font, 'B', size=code_size)
...
def draw_toc(pdf, toc_data, user_info, page_offset=0):
    ...
    pdf.set_font(pdf.code_font, 'B', 8)
...
def draw_algorithms(pdf, library_data, lexer, save_toc=False):
    ...
    # Adicionado 'i' e 'enumerate' para saber quando trocar de página
    for i, (category_name, files) in enumerate(sorted(library_data.items())):
        
        # Se NÃO for a primeira pasta (i > 0), pula para uma nova página
        if i > 0:
            pdf.add_page()
            pdf.set_column(0)
            pdf.col_start_y = pdf.top_margin
...
def scan_library_files():
    ...
    root_directory = Path(__file__).resolve().parent.parent / "FuncRepositoryCpp"
